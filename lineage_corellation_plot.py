...
trackData = trackData.drop(trackData[(trackData.LEN < 2)].index) #remove all cycles below 10 frames length
trackData = trackData.drop(trackData[(trackData.LEN > 100)].index) #remove overlong cycles (for purpose we are interested in)
trackData = trackData.drop(trackData[(trackData.GEN > 5)].index)  #remove generations above 3 as only 1 and 2 are useful in this particular dataset. this will vary between datasets. restrict  to full cell cycle stages (or near full)
#print(trackData)
...
gen1=trackData["GEN"] == gen_look_1
gen2=trackData["GEN"] == gen_look_2
gen3=trackData["GEN"] == gen_look_3
...
for a in g1x2list:
	if "x3" in a[0]:
		continue
	print(a)
	plt.scatter(a[1]*0.95,a[2]*0.95,c='black',alpha=0.1)
plt.title("Correlation between generation {} and generation {} from dataset TLAB".format(gen_look_1+1,gen_look_2+1))	
plt.xlabel("generation {} time (mins)".format(gen_look_1+1))
plt.ylabel("generation {} time (mins)".format(gen_look_2+1))
plt.show()
